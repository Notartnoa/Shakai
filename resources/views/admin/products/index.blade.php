    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.products.create') }}"
               class="inline-flex items-center gap-2 px-4 py-2 bg-[#2D68F8] hover:bg-[#083297] text-white font-medium rounded-xl transition-colors">
            ...
            <div class="flex items-center gap-5 p-5 {{ !$loop->last ? 'border-b border-[#2A2A2A]' : '' }} hover:bg-[#2A2A2A]/50 transition-colors">
                ...
                <div class="flex items-center gap-2 flex-shrink-0">
                    <a href="{{ route('front.details', $product->slug) }}"
                       target="_blank"
                       class="p-2 text-[#595959] hover:text-white hover:bg-[#2A2A2A] rounded-lg transition-colors"
                       title="View">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="{{ route('admin.products.edit', $product->id) }}"
                       class="p-2 text-[#595959] hover:text-[#2D68F8] hover:bg-[#2D68F8]/10 rounded-lg transition-colors"
                       title="Edit">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <form action="{{ route('admin.products.destroy', $product) }}" method="POST"
                        ...
                        @method('DELETE')
                        <button type="submit"
                                class="p-2 text-[#595959] hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-colors"
                                title="Delete">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
